...
final class DataModel: ObservableObject {
    ...
    private func unpackPhoto(_ photo: AVCapturePhoto) -> PhotoData? {
        ...
        // Create a UIImage from the photo data
        guard let photoImage = UIImage(data: imageData) else { return nil }
        
        // Load the logo image
        guard let logo = UIImage(named: "Century21") else { return nil }
        
        // Begin a new image context
        UIGraphicsBeginImageContextWithOptions(photoImage.size, false, 0.0)
        
        // Draw the original photo
        photoImage.draw(in: CGRect(origin: CGPoint.zero, size: photoImage.size))
        
        // Calculate the position and size for the logo
        let logoScaleFactor: CGFloat = 0.65 // Adjust size scale as needed
        let logoSize = CGSize(width: photoImage.size.width * logoScaleFactor, height: logo.size.height * (photoImage.size.width * logoScaleFactor / logo.size.width))
        let logoOrigin = CGPoint(x: (photoImage.size.width - logoSize.width) / 2, y: (photoImage.size.height - logoSize.height) / 2)
        
        // Draw the logo on top of the photo
        logo.draw(in: CGRect(origin: logoOrigin, size: logoSize), blendMode: .normal, alpha: 0.6) // Adjust alpha as needed
        
        // Get the final image with the logo
        let finalImage = UIGraphicsGetImageFromCurrentImageContext()
        
        // End the image context
        UIGraphicsEndImageContext()
        
        // Convert the final image back to data
        guard let finalImageData = finalImage?.jpegData(compressionQuality: 1.0) else { return nil }
        
        return PhotoData(thumbnailImage: thumbnailImage, thumbnailSize: thumbnailSize, imageData: finalImageData, imageSize: imageSize)
    }
    /*#-code-walkthrough(photoflow.unpackPhoto)*/
